                ...
                <div class="col-md-3">
                    <div class="input-group">
                        ...
                        <select class="form-control" name="position_id">
                            <?php if(is_array($posIds)): foreach($posIds as $key=>$posId): ?><option value="<?php echo ($posId["id"]); ?>" <?php if($posId["id"] == $positionId): ?>selected="selected"<?php endif; ?>><?php echo ($posId["name"]); ?></option><?php endforeach; endif; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="col-lg-6">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-hover singcms-table">
                        ...
                        <tbody>
                            <?php if(is_array($posContents)): $i = 0; $__LIST__ = $posContents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$posContent): $mod = ($i % 2 );++$i;?><tr>
                                <td><input size=4 type='text'  name='' value=""/></td>
                                <td><?php echo ($posContent["id"]); ?></td>
                                <td><?php echo ($posContent["title"]); ?></td>
                                <td><?php echo (date("Y-m-d H:i:s",$posContent["create_time"])); ?></td>
                                <td><?php echo (isThumb($posContent["thumb"])); ?></td>
                                <td>
                                    <?php echo (status($posContent["status"])); ?>
                                </td>
                                <td>
                                    <span class="sing_cursor glyphicon glyphicon-edit" aria-hidden="true" id="singcms-edit" attr-id="<?php echo ($posContent["id"]); ?>" >编辑</span>
